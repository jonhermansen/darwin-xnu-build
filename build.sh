...
function setup_xcode_toolchain() {
    ...
    export PATH="${XCODE_TOOLCHAIN}:${XCODE_USR_BIN}:${EXTRA_PATH:+${EXTRA_PATH}:}${FAKEROOT_DIR}/usr/local/bin:${FAKEROOT_DIR}/usr/local/libexec:${SYSTEM_PATHS}${HOMEBREW_PATHS}"
...
choose_xnu() {
    if [ -z "$MACOS_VERSION" ]; then
        ...
        MACOS_VERSION=$(gum choose "12.5" "13.0" "13.1" "13.2" "13.3" "13.4" "13.5" "14.0" "14.1" "14.2" "14.3" "14.4" "14.5" "14.6" "15.0" "15.1" "15.2" "15.3" "15.4" "15.5" "15.6" "26.0" "26.1" "26.2" "26.3" "26.4")
    fi
    ...
    '26.3')
        ...
        KDK_NAME='Kernel Debug Kit 26.3 build 25D5087f'
        KDKROOT='/Library/Developer/KDKs/KDK_26.3_25D5087f.kdk/'
...
patches() {
    ...
    # specify location of ctf tools
    sed -i 's|export CTFCONVERT := $(shell $(XCRUN) -sdk $(SDKROOT) -find ctfconvert.*)|export CTFCONVERT := $(shell find $(FAKEROOT_DIR) -name "ctfconvert" -type f)|g' "${WORK_DIR}/xnu/makedefs/MakeInc.cmd"
    sed -i 's|export CTFMERGE := $(shell $(XCRUN) -sdk $(SDKROOT) -find ctfmerge.*)|export CTFMERGE := $(shell find $(FAKEROOT_DIR) -name "ctfmerge" -type f)|g' "${WORK_DIR}/xnu/makedefs/MakeInc.cmd"
    sed -i 's|export CTFDUMP := $(shell $(XCRUN) -sdk $(SDKROOT) -find ctfdump.*)|export CTFDUMP := $(shell find $(FAKEROOT_DIR) -name "ctfdump" -type f)|g' "${WORK_DIR}/xnu/makedefs/MakeInc.cmd"
    sed -i 's|export CTFINSERT := $(shell $(XCRUN) -sdk $(SDKROOT) -find ctf_insert.*)|export CTFINSERT := $(shell find $(FAKEROOT_DIR) -name "ctf_insert" -type f)|g' "${WORK_DIR}/xnu/makedefs/MakeInc.cmd"
...
build_xnu() {
    if [ ! -f "${BUILD_DIR}/xnu.obj/kernel.${KERNEL_TYPE}" ]; then
        if [ "$JSONDB" -ne "0" ]; then
            ...
            cd "${SRCROOT}"
            make install -j8 VERBOSE=YES SDKROOT=macosx TARGET_CONFIGS="$KERNEL_CONFIG $ARCH_CONFIG $MACHINE_CONFIG" CONCISE=0 LOGCOLORS=y BUILD_WERROR=0 BUILD_LTO=1 SRCROOT="${SRCROOT}" OBJROOT="${OBJROOT}" SYMROOT="${SYMROOT}" DSTROOT="${DSTROOT}" FAKEROOT_DIR="${FAKEROOT_DIR}" KDKROOT="${KDKROOT}" TIGHTBEAMC=${TIGHTBEAMC} RC_DARWIN_KERNEL_VERSION=${RC_DARWIN_KERNEL_VERSION} MEMORY_SIZE="${MEMORY_SIZE_OVERRIDE}" SYSCTL_HW_PHYSICALCPU="${PHYS_CPU_OVERRIDE}" SYSCTL_HW_LOGICALCPU="${LOGICAL_CPU_OVERRIDE}" KERNEL_BUILDS_IN_PARALLEL="${KERNEL_PARALLELISM_OVERRIDE:-1}"
            cd "${WORK_DIR}"
        fi
    else
